...
interface ParsedArgs {
  ...
  positionals: string[];
  /**
   * Named flags captured from `--name value` or `--name=value`.
   * 从 `--name value` 或 `--name=value` 捕获的命名标志。
   */
 ...
 * 从 CLI 标志构造运行时资产安装选项。
 */
function runtimeInstallOptionsFromArgs(parsed: ParsedArgs, runtimeRoot: string): RuntimeInstallOptions {
  return {
    runtimeRoot,
    database: normalizeDatabasePreset((stringFlag(parsed, "database") as RuntimeDatabasePreset | undefined) ?? RuntimeDatabasePreset.None),
    luaskillsVersion: stringFlag(parsed, "luaskills-version"),
    luaRuntimeVersion: stringFlag(parsed, "lua-runtime-version"),
    ...
    includeLuaRuntime: !booleanFlag(parsed, "skip-lua-runtime"),
    luaskillsRepo: stringFlag(parsed, "luaskills-repo"),
    luaRuntimeRepo: stringFlag(parsed, "lua-runtime-repo"),
 ...
 * 分发一个需要引擎句柄的命令。
 */
async function dispatchEngineCommand(
  ...
  switch (command) {
    ...
    case "run-lua":
      printJson(
        client.runLua(
          stringFlag(parsed, "code") ?? requireValue(first, "run-lua requires <code> [args-json]"),
          parseJsonValue(second ?? stringFlag(parsed, "args") ?? "{}"),
        ),
      );
      return;
    case "config":
    ...
    case "uninstall":
      printJson(
        client.skills.uninstall(
          skillRoots,
          requireValue(first, "uninstall requires <skill-id>"),
          uninstallOptionsFromArgs(parsed),
          lifecycleOptionsFromArgs(parsed, skillRoots),
        ),
      );
      return;
    case "system-disable":
    ...
    case "system-uninstall":
      printJson(
        client.system(authority).uninstall(
          skillRoots,
          requireValue(first, "system-uninstall requires <skill-id>"),
          uninstallOptionsFromArgs(parsed),
          lifecycleOptionsFromArgs(parsed, skillRoots),
        ),
      );
 ...
 * 从 CLI 标志构造正式 root 链。
 */
function rootsFromArgs(parsed: ParsedArgs, runtimeRoot: string): RuntimeSkillRoot[] {
 ...
 */
function replaceRootDir(roots: RuntimeSkillRoot[], label: string, value?: string): void {
  if (!value) {
    return;
  }
 ...
 * 从 CLI 标志构造生命周期选项。
 */
function lifecycleOptionsFromArgs(parsed: ParsedArgs, skillRoots: RuntimeSkillRoot[]): { targetRoot?: RuntimeSkillRoot; authority?: Authority | `${Authority}` } {
 ...
 * 从 CLI 标志构造卸载清理选项。
 */
function uninstallOptionsFromArgs(parsed: ParsedArgs): { remove_sqlite: boolean; remove_lancedb: boolean } {
  return {
 ...
 * 返回选中的宿主注入权限。
 */
function authorityFlag(parsed: ParsedArgs): Authority | `${Authority}` {
 ...
 * 要求单个位置参数存在并返回该值。
 */
function requireValue<T extends string | undefined>(value: T, message: string): string {
  if (!value) {
